...
app.get('/api/products/low-stock', async (_req, res) => {
  try {
    ok(res, await query('SELECT * FROM products WHERE stock <= reorder_level ORDER BY stock ASC'));
  } catch (e) { fail(res, e.message, 500); }
});
...
app.delete('/api/held-bills/:id', async (req, res) => {
  try {
    await run('DELETE FROM held_bills WHERE id=?', [req.params.id]);
    ok(res, { message: 'Held bill removed' });
  } catch (e) { fail(res, e.message, 500); }
});

/* ============================================================
   CHECKOUT (with negative-stock block + movement log)
   ============================================================ */
app.post('/api/checkout', async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      if (!allowNeg && p.stock < item.quantity) {
...
app.get('/api/backup', async (_req, res) => {
  try {
    const [products, sales, saleItems, customers, suppliers, expenses, settings, stockMovements, heldBills] = await Promise.all([
    ...
    const snapshot = {
      ...
      version: 2,
      data: { products, sales, saleItems, customers, suppliers, expenses, settings, stockMovements, heldBills },
    };
